...
| Chat.Model.Name                                 | string           | null      | Specifies the chat model to use (e.g., GPT-4.1). Must be configured in the chosen AI provider.                |
...
| `Model.Name`           | string   | `null`     | Name of the embedding model (must be supported by the chosen AI provider).           |
...
| Metadata.Model               |          |           | Chat model configuration for metadata extraction. The model configuration is identical to [Chat Model Configuration](#-chat-configuration-details) |
