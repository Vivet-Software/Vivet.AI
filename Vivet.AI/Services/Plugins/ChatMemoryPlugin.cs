...
/// Memory Plugin
/// </summary>
public sealed class ChatMemoryPlugin : BaseMemoryPlugin
{
    /// <summary>
    /// Constructor.
    /// </summary>
    /// <param name="embeddingMemoryService">The <see cref="IEmbeddingMemoryService"/>.</param>
    public ChatMemoryPlugin(IEmbeddingMemoryService embeddingMemoryService)
...
and inject them into the current chat context to support personalized and consistent responses.")]
    public async Task<string> GetMemoriesAsync([Description("The current user question or message")]string question, ChatMemoryContext context, MemorySearchConfigOverrides configOverrides, CancellationToken cancellationToken = default)
    {
        if (context == null) 
