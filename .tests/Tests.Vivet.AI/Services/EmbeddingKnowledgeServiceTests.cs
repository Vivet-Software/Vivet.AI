using System;
using System.Linq;
using System.Threading.Tasks;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using Newtonsoft.Json;
using Tests.Vivet.AI.Services.data;
using Vivet.AI.Services.Exceptions;
using Vivet.AI.Services.Interfaces;
using Vivet.AI.Services.Models;
using Vivet.AI.Services.Models.Blobs;
using Vivet.AI.Services.Models.Blobs.Data;
using Vivet.AI.Services.Models.MimeTypes;
using Vivet.AI.Services.Requests.Embedding;
using Vivet.AI.Services.Requests.Embedding.Knowledge;
using Vivet.AI.Services.Requests.Embedding.Knowledge.Enums;
using Vivet.AI.Services.Serialization;

namespace Tests.Vivet.AI.Services;

[TestClass]
public class EmbeddingKnowledgeServiceTests : BaseTests
{
    private IEmbeddingKnowledgeService EmbeddingKnowledgeService => this.serviceProvider.GetRequiredService<IEmbeddingKnowledgeService>();

    internal sealed class JsonClass
    {
        public bool SummerAlways { get; set; }
    }
    
    [TestMethod]
    public async Task IndexTextTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string TEXT = "The apple is black and old.";

        var indexRequest = new IndexTextRequest
        {
            Text = TEXT,
            TenantId = this.tenantId,
            SubTenantId = this.subTenantId,
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            CreatedBy = this.createdBy,
            Source = this.source,
            Tags =
            [
                "tag1",
                "tag2"
            ]
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);
        Assert.AreEqual(1, indexResponse.TotalEmbeddings);
        Assert.AreEqual(27, indexResponse.TotalEmbeddingsSize);
        Assert.IsNotNull(indexResponse.TokenUsage);
        Assert.AreEqual(7, indexResponse.TokenUsage.InputTokens);
        Assert.IsNull(indexResponse.TokenUsage.OutputTokens);
        Assert.IsNull(indexResponse.MetadataTokenUsage);

        var knowledgeResponses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = knowledgeResponses.Results.ToArray();

        Assert.AreEqual(1, results.Length);

        var knowledgeResponse = results
            .FirstOrDefault();

        Assert.IsNotNull(knowledgeResponse);
        Assert.IsTrue(knowledgeResponse.Size > 0);
        Assert.AreEqual(indexRequest.Text, knowledgeResponse.Result.Content);
        Assert.AreEqual(indexRequest.TenantId, knowledgeResponse.Result.TenantId);
        Assert.AreEqual(indexRequest.SubTenantId, knowledgeResponse.Result.SubTenantId);
        Assert.AreEqual(indexRequest.ScopeId, knowledgeResponse.Result.ScopeId);
        Assert.AreEqual(indexRequest.UserId, knowledgeResponse.Result.UserId);
        Assert.AreEqual(indexRequest.Language, knowledgeResponse.Result.Language);
        Assert.AreEqual(indexRequest.CreatedBy, knowledgeResponse.Result.CreatedBy);
        Assert.AreEqual(indexRequest.Source, knowledgeResponse.Result.Source);
        CollectionAssert.AreEquivalent(indexRequest.Tags, knowledgeResponse.Result.Tags);
    }

    [TestMethod]
    public async Task IndexTextWhenTTest()
    {
        var scopeId = Guid.NewGuid().ToString();

        var indexRequest = new IndexTextRequest<JsonClass>
        {
            Text = new JsonClass
            {
                SummerAlways = true
            },
            ScopeId = scopeId
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);
        Assert.AreEqual(1, indexResponse.TotalEmbeddings);
        Assert.AreEqual(21, indexResponse.TotalEmbeddingsSize);
        Assert.IsNotNull(indexResponse.TokenUsage);
        Assert.AreEqual(6, indexResponse.TokenUsage.InputTokens);
        Assert.IsNull(indexResponse.TokenUsage.OutputTokens);
        Assert.IsNull(indexResponse.MetadataTokenUsage);

        var knowledgeResponses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = knowledgeResponses.Results.ToArray();

        Assert.AreEqual(1, results.Length);

        var knowledgeResponse = results
            .FirstOrDefault();

        var serializeObject = JsonConvert.SerializeObject(indexRequest.Text, Formatting.None, Settings.SerializerSettings);

        Assert.IsNotNull(knowledgeResponse);
        Assert.IsTrue(knowledgeResponse.Size > 0);
        Assert.AreEqual(serializeObject, knowledgeResponse.Result.Content);
        Assert.AreEqual(indexRequest.ScopeId, knowledgeResponse.Result.ScopeId);
        CollectionAssert.AreEquivalent(indexRequest.Tags, knowledgeResponse.Result.Tags);
    }

    [TestMethod]
    public async Task IndexTextWhenJsonTextTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task IndexTextWhenXmlTextTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task IndexTextWhenExistingContentHashMatchTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string TEXT = "The record already exists in the vector store.";

        await this.EmbeddingKnowledgeService
            .IndexAsync(new IndexTextRequest
            {
                Text = TEXT,
                ScopeId = scopeId
            });

        var response = await this.EmbeddingKnowledgeService
            .IndexAsync(new IndexTextRequest
            {
                Text = TEXT,
                ScopeId = scopeId
            });

        Assert.AreEqual(0, response.TotalEmbeddings);
        Assert.AreEqual(0, response.TotalEmbeddingsSize);
        Assert.IsNull(response.TokenUsage);
        Assert.IsNull(response.MetadataTokenUsage);
    }

    [TestMethod]
    public async Task IndexImageTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var request = new IndexImageRequest
        {
            TenantId = this.tenantId,
            UserId = this.userId,
            ScopeId = scopeId,
            Blob = new ImageBlob
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg,
                Metadata = new Metadata
                {
                    Summary = "summary",
                    Description = "description"
                }
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(request);

        Assert.IsNotNull(indexResponse);
        Assert.IsNull(indexResponse.MetadataTokenUsage);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = responses.Results.ToArray();

        Assert.AreEqual(1, results.Length);

        var result = results.FirstOrDefault();

        Assert.IsNotNull(result);
        Assert.IsTrue(result.Size > 0);
        Assert.AreEqual("summary", result.Result.Content);
        Assert.AreEqual("description", result.Result.FullContext);
        Assert.IsNull(result.Result.BlobMetadata);
    }

    [TestMethod]
    public async Task IndexImageWhenMetadataRetievalTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var request = new IndexImageRequest
        {
            TenantId = this.tenantId,
            UserId = this.userId,
            ScopeId = scopeId,
            Blob = new ImageBlob
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(request);

        Assert.IsNotNull(indexResponse);
        Assert.AreEqual(1, indexResponse.TotalEmbeddings);
        Assert.IsTrue(indexResponse.TotalEmbeddingsSize > 2500);
        Assert.IsNotNull(indexResponse.TokenUsage);
        Assert.IsTrue(indexResponse.TokenUsage.InputTokens >= 8);
        Assert.IsNull(indexResponse.TokenUsage.OutputTokens);
        Assert.IsNotNull(indexResponse.MetadataTokenUsage);
        Assert.IsTrue(indexResponse.MetadataTokenUsage.InputTokens > 275);
        Assert.IsTrue(indexResponse.MetadataTokenUsage.OutputTokens >= 50);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = responses.Results.ToArray();

        Assert.AreEqual(1, results.Length);

        var result = results.FirstOrDefault();

        Assert.IsNotNull(result);
        Assert.IsTrue(result.Size > 0);
        Assert.IsTrue(result.Result.Content.ToLower().Contains("darth vader"));
        Assert.IsTrue(result.Result.Content.ToLower().Contains("mask") || result.Result.Content.ToLower().Contains("helmet"));
        Assert.IsTrue(result.Result.FullContext.ToLower().Contains("darth vader"));
        Assert.IsTrue(result.Result.FullContext.ToLower().Contains("mask") || result.Result.FullContext.ToLower().Contains("helmet"));
        Assert.IsNull(result.Result.BlobMetadata);
    }

    [TestMethod]
    public async Task IndexImageWhenAdditionalMetadataTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var indexRequest = new IndexImageRequest
        {
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            Blob = new ImageBlob<ImageMetaData>
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg,
                Metadata = new Metadata
                {
                    Summary = "summary",
                    Description = "description"
                },
                AdditionalMetadata = new ImageMetaData
                {
                    Width = 1,
                    Height = 1,
                    Resolution = "1x1"
                }
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);
        Assert.IsNull(indexResponse.MetadataTokenUsage);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var result = responses.Results.First(x => x.Result.Blob != null);

        Assert.IsNotNull(result);
        Assert.AreEqual("summary", result.Result.Content);
        Assert.AreEqual("description", result.Result.FullContext);
        Assert.IsNotNull(result.Result.BlobMetadata);

        var imageMetaData = result.Result.BlobMetadata.ToObject<ImageMetaData>();
        Assert.AreEqual(1, imageMetaData.Width);
        Assert.AreEqual(1, imageMetaData.Height);
        Assert.AreEqual("1x1", imageMetaData.Resolution);
    }

    [TestMethod]
    public async Task IndexImageWhenAdditionalMetadataRetievalTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var indexRequest = new IndexImageRequest
        {
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            Blob = new ImageBlob<ImageMetaData>
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg,
                Metadata = new Metadata
                {
                    Summary = "summary",
                    Description = "description"
                }
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var result = responses.Results.First(x => x.Result.Blob != null);

        Assert.IsNotNull(result);
        Assert.AreEqual("summary", result.Result.Content);
        Assert.AreEqual("description", result.Result.FullContext);
        Assert.IsNotNull(result.Result.BlobMetadata);

        var imageMetaData = result.Result.BlobMetadata.ToObject<ImageMetaData>();
        Assert.IsNotNull(imageMetaData.Width);
        Assert.IsNotNull(imageMetaData.Height);
        Assert.IsNotNull(imageMetaData.Resolution);
    }

    [TestMethod]
    public async Task IndexImageWhenAdditionalMetadataAndMetadataRetievalTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var indexRequest = new IndexImageRequest
        {
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            Blob = new ImageBlob<ImageMetaData>
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg,
                AdditionalMetadata = new ImageMetaData
                {
                    Width = 1,
                    Height = 1,
                    Resolution = "1x1"
                }
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var result = responses.Results.First(x => x.Result.Blob != null);

        Assert.IsNotNull(result);
        Assert.IsNotNull(result.Result.BlobMetadata);
        Assert.IsTrue(result.Result.Content.ToLower().Contains("darth vader"));
        Assert.IsTrue(result.Result.Content.ToLower().Contains("mask") || result.Result.Content.ToLower().Contains("helmet"));
        Assert.IsTrue(result.Result.FullContext.ToLower().Contains("darth vader") && result.Result.FullContext.ToLower().Contains("helmet"));
        Assert.IsTrue(result.Result.FullContext.ToLower().Contains("mask") || result.Result.FullContext.ToLower().Contains("helmet"));
        Assert.IsNotNull(result.Result.BlobMetadata);

        var imageMetaData = result.Result.BlobMetadata.ToObject<ImageMetaData>();
        Assert.AreEqual(1, imageMetaData.Width);
        Assert.AreEqual(1, imageMetaData.Height);
        Assert.AreEqual("1x1", imageMetaData.Resolution);
    }

    [TestMethod]
    public async Task IndexImageWhenMetadataAndAdditionalMetadataRetievalTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var indexRequest = new IndexImageRequest
        {
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            Blob = new ImageBlob<ImageMetaData>
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(indexRequest);

        Assert.IsNotNull(indexResponse);

        var responses = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var result = responses.Results.First(x => x.Result.Blob != null);

        Assert.IsNotNull(result);
        Assert.IsNotNull(indexResponse.MetadataTokenUsage);
        Assert.IsTrue(indexResponse.MetadataTokenUsage.InputTokens >= 350);
        Assert.IsTrue(indexResponse.MetadataTokenUsage.OutputTokens >= 50);
        Assert.IsTrue(result.Result.Content.ToLower().Contains("darth vader"));
        Assert.IsTrue(result.Result.Content.ToLower().Contains("helmet") || result.Result.Content.ToLower().Contains("mask"));
        Assert.IsTrue(result.Result.FullContext.ToLower().Contains("helmet") || result.Result.FullContext.ToLower().Contains("mask"));
        Assert.IsNotNull(result.Result.BlobMetadata);

        var imageMetaData = result.Result.BlobMetadata.ToObject<ImageMetaData>();
        Assert.IsNotNull(imageMetaData.Width);
        Assert.IsNotNull(imageMetaData.Height);
        Assert.IsNotNull(imageMetaData.Resolution);
    }

    [TestMethod]
    public async Task IndexImageWhenNoRequestMetadataAndUseAutomaticMetadataRetrievalIsFalseThrowsAiExceptionTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var indexRequest = new IndexImageRequest
        {
            ScopeId = scopeId,
            UserId = this.userId,
            Language = this.language,
            Blob = new ImageBlob<ImageMetaData>
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg
            },
            ConfigOverrides =
            {
                Metadata =
                {
                    UseAutomaticMetadataRetrieval = false
                }
            }
        };

        await Assert.ThrowsAsync<AiException>(async () => await this.EmbeddingKnowledgeService.IndexAsync(indexRequest));
    }

    [TestMethod]
    public async Task IndexAudioTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task IndexVideoTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task IndexDocumentTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task QueryTextTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string TEXT = "The apple is black and old.";

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(new IndexTextRequest
            {
                Text = TEXT,
                TenantId = this.tenantId,
                SubTenantId = this.subTenantId,
                ScopeId = scopeId,
                UserId = this.userId,
                Language = this.language,
                CreatedBy = this.createdBy,
                Source = this.source,
                Tags =
                [
                    "tag1",
                    "tag2"
                ]
            });

        Assert.AreEqual(1, indexResponse.TotalEmbeddings);
        Assert.AreEqual(27, indexResponse.TotalEmbeddingsSize);
        Assert.IsNotNull(indexResponse.TokenUsage);
        Assert.AreEqual(7, indexResponse.TokenUsage.InputTokens);
        Assert.IsNull(indexResponse.TokenUsage.OutputTokens);
        Assert.IsNull(indexResponse.MetadataTokenUsage);

        var queryResponse = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    TenantId = this.tenantId,
                    SubTenantId = this.subTenantId,
                    ScopeId = scopeId,
                    UserId = this.userId,
                    Tag = "tag2",
                    DateRange = new DateRange
                    {
                        FromAt = DateTimeOffset.UtcNow.AddHours(-1),
                        ToAt = DateTimeOffset.UtcNow.AddHours(1)
                    },
                    Language = this.language,
                    SearchFor = SearchFor.Text
                },
                Limit = 10
            });

        Assert.IsNotNull(queryResponse);
        Assert.AreEqual(1, queryResponse.Results.Count());
    }

    [TestMethod]
    public async Task QueryTextWhenNoCriteriasTest()
    {
        const string TEXT = "The apple is black and old.";

        for (var i = 0; i < 2; i++)
        {
            await this.EmbeddingKnowledgeService
                .IndexAsync(new IndexTextRequest
                {
                    Text = TEXT,
                    ScopeId = Guid.NewGuid().ToString()
                });
        }

        var response = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Limit = 10
            });

        Assert.IsNotNull(response);
        Assert.IsTrue(response.Results.Count() >= 2);
    }

    [TestMethod]
    public async Task QueryTextWhenSkipTest()
    {
        var localUserId = Guid.NewGuid().ToString();
        const string TEXT = "The apple is black and old.";

        for (var i = 0; i < 2; i++)
        {
            await this.EmbeddingKnowledgeService
                .IndexAsync(new IndexTextRequest
                {
                    Text = TEXT,
                    ScopeId = Guid.NewGuid().ToString(),
                    UserId = localUserId
                });
        }

        var response = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    UserId = localUserId
                },
                Limit = 10,
                Skip = 1
            });

        Assert.IsNotNull(response);
        Assert.AreEqual(1, response.Results.Count());
    }

    [TestMethod]
    public async Task QueryTextWhenOrderByUnixTimeStampTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string TEXT = "The apple is black and old.";

        for (var i = 0; i < 2; i++)
        {
            await this.EmbeddingKnowledgeService
                .IndexAsync(new IndexTextRequest
                {
                    Text = $"{i}. {TEXT}",
                    ScopeId = scopeId
                });

            await Task.Delay(TimeSpan.FromSeconds(1));
        }

        var response = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = response.Results.ToArray();

        Assert.IsNotNull(response);
        Assert.AreEqual(2, results.Length);

        var result = results.FirstOrDefault();
        Assert.IsNotNull(result);
        Assert.IsTrue(result.Result.Content.StartsWith("0. "));
    }

    [TestMethod]
    public async Task SearchTest()
    {
        var scopeId = Guid.NewGuid().ToString();

        await this.EmbeddingKnowledgeService
            .IndexAsync(new IndexTextRequest
            {
                Text = "The weather in Sweden is cold.",
                TenantId = this.tenantId,
                SubTenantId = this.subTenantId,
                ScopeId = scopeId,
                UserId = this.userId,
                Language = this.language,
                CreatedBy = this.createdBy,
                Source = this.source,
                Tags =
                [
                    "tag1",
                    "tag2"
                ]
            });

        var response = await this.EmbeddingKnowledgeService
            .SearchAsync(new SearchKnowledgeRequest
            {
                Query = "What is the weather in Sweden?",
                Criteria =
                {
                    TenantId = this.tenantId,
                    SubTenantId = this.subTenantId,
                    UserId = this.userId,
                    ScopeId = scopeId
                },
                Limit = 1
            });

        var result = response.Results
            .FirstOrDefault();

        Assert.IsNotNull(result);
        Assert.IsNotNull(result.Result);
        Assert.AreEqual("The weather in Sweden is cold.", result.Result.Content);
    }

    [TestMethod]
    public async Task SearchWhenNoCriteriasTest()
    {
        const string TEXT = "The apple is black and old.";

        for (var i = 0; i < 2; i++)
        {
            await this.EmbeddingKnowledgeService
                .IndexAsync(new IndexTextRequest
                {
                    Text = TEXT,
                    ScopeId = Guid.NewGuid().ToString()
                });
        }

        var response = await this.EmbeddingKnowledgeService
            .SearchAsync(new SearchKnowledgeRequest
            {
                Query = TEXT,
                Limit = 10
            });

        Assert.IsNotNull(response);
        Assert.IsTrue(response.Results.Count() >= 2);
    }

    [TestMethod]
    public async Task SearchWhenBlobTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string BASE64 = "";

        var request = new IndexImageRequest
        {
            TenantId = this.tenantId,
            UserId = this.userId,
            ScopeId = scopeId,
            Blob = new ImageBlob
            {
                Data = new BlobDataBase64
                {
                    Base64 = BASE64
                },
                MimeType = ImageMimeType.Jpg,
                Metadata = new Metadata
                {
                    Summary = "summary",
                    Description = "description"
                }
            }
        };

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(request);

        Assert.IsNotNull(indexResponse);

        var responses = await this.EmbeddingKnowledgeService
            .SearchAsync(new SearchKnowledgeRequest
            {
                Query = "summary",
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        var results = responses.Results.ToArray();

        Assert.AreEqual(1, results.Length);

        var result = results.FirstOrDefault();

        Assert.IsNotNull(result);
        Assert.AreEqual("summary", result.Result.Content);
        Assert.AreEqual("description", result.Result.FullContext);
        Assert.IsNull(result.Result.BlobMetadata);
    }

    [TestMethod]
    public async Task SearchWhenRecencyScoreTest()
    {
        await Task.CompletedTask;
        Assert.Inconclusive();
    }

    [TestMethod]
    public async Task DeleteTest()
    {
        var scopeId = Guid.NewGuid().ToString();
        const string TEXT = "The apple is black and old.";

        var indexResponse = await this.EmbeddingKnowledgeService
            .IndexAsync(new IndexTextRequest
            {
                Text = TEXT,
                ScopeId = scopeId
            });

        Assert.IsNotNull(indexResponse);

        var queryResponse = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        Assert.IsNotNull(queryResponse);

        var queryResult = queryResponse.Results
            .FirstOrDefault();

        Assert.IsNotNull(queryResult);

        await this.EmbeddingKnowledgeService
            .DeleteAsync(new DeleteRequest
            {
                Ids =
                [
                    queryResult.Result.Id
                ]
            });

        var queryResponse2 = await this.EmbeddingKnowledgeService
            .QueryAsync(new QueryKnowledgeRequest
            {
                Criteria =
                {
                    ScopeId = scopeId
                },
                Limit = 10
            });

        Assert.IsNotNull(queryResponse2);
        Assert.AreEqual(0, queryResponse2.Results.Count());
    }
}